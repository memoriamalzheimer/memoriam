  ...
  if (cartasViradas.length === 2) {
    ...
    if (c1.dataset.imagem === c2.dataset.imagem) {
      ...
      if (paresEncontrados === totalPares) {
        pararCronometro();
        fetch('/api/jogos/resultado', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({jogo:'Jogo da Memória', tempo: tempo, tentativas})}).catch(e=>console.error(e));
